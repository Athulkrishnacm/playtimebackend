                    ...
                    bcrypt.compare(password, turfOwner.password, (err, result) => {
                        if (result) {
                            ...
                            if (turfOwner.verification === 'Success') {
                                ...
                                console.log("error23333");
                                response.status = 'Pending'
                                res.status(200).json(response)
                            } else if (turfOwner.verification === 'Rejected') {
                                console.log("error433445");
    ...
    try {
        if (token) {
            ...
            jwt.verify(token, process.env.TOKEN_SECRET, async (err, result) => {
                console.log("cvafl aodsgn iodghh'p");
                if (!err) {
                    console.log("asdfghjl;");
                    let user = await turfModel.findOne({ _id: result.turfOwnerId })
                    console.log("qawsedrftg");
                    if (user) {
                        console.log("qwrum vvjnvm vivdm");
                        if (!user.block) {
                            console.log("qawsedrfvxcx");
                            res.status(200).json({ authorization: true })
                        } else {
                            console.log("qawdkgbowubgsedrfvxcx");
                            res.status(401).json({ authorization: false })
                        }
                    } else {
                        console.log("qawsndt8yqevfqipofh");
                        res.status(401).json({ authorization: false })
                    }
                } else {
                    console.log("qwsdfvbnoijhbdvkjbdwv");
                    res.status(401).json({ authorization: false })
                }
            })
        } else {
            console.log("1234567890098765432");
            res.status(401).json({ authorization: false })
        }
    }
    catch (err) {
